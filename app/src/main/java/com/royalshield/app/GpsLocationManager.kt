    ...
    private fun sendLocationToServer(latitude: Double, longitude: Double) {
        scheduler.execute {
            val json = JSONObject()
                ...
                .put("source", "android_gps")
                .toString()
            val body = json.toRequestBody("application/json; charset=utf-8".toMediaTypeOrNull())
            val endpoint = "${BuildConfig.API_BASE_URL.trimEnd('/')}/api/location/track"
            ...
            try {
                client.newCall(request).execute().use { response ->
                    if (!response.isSuccessful) Log.e("GpsLocationManager", "Location sync failed: ${response.code}")
                }
            } catch (e: Exception) {
